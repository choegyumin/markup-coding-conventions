...
작업 진행에 참고해야 하는 주석은 맨 앞에 `[D]`를 추가하여 작성한다.
...
컨테이너는 컴포넌트의 일종이며 하위 컴포넌트를 포함한 완성된 UI 세트를 말한다. 이는 BEMIT 방법론이 아닌 React 디자인 패턴에서 따온 것이다.  
컨테이너는 컴포넌트 뿐만 아니라 오브젝트도 포함할 수 있으며, **컨테이너끼리는 절대 서로 조합하여서는 안된다.**  
